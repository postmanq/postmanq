...
func (s *EventSenderTestSuite) TestSendEvent() {
	middlewareActivityType := randomdata.Alphanumeric(32)
	middlewareActivityDescriptor := temporal_mocks.NewMockActivityDescriptor(s.Ctrl)
	middlewareActivityDescriptor.EXPECT().GetActivityType().AnyTimes().Return(middlewareActivityType)
	s.middlewarePlugin.EXPECT().GetActivityDescriptor().AnyTimes().Return(middlewareActivityDescriptor)
	...
	s.activityExecutorFactory.EXPECT().Create(middlewareActivityType).AnyTimes().Return(middlewareActivityExecutor)
	...
	err := s.sender.SendEvent(nil, &postmanqv1.Event{})
	s.NotNil(err)
	...
	senderActivityType := randomdata.Alphanumeric(32)
	senderActivityDescriptor := temporal_mocks.NewMockActivityDescriptor(s.Ctrl)
	senderActivityDescriptor.EXPECT().GetActivityType().AnyTimes().Return(senderActivityType)
	s.senderPlugin.EXPECT().GetActivityDescriptor().AnyTimes().Return(senderActivityDescriptor)
	senderActivityExecutor := temporal_mocks.NewMockActivityExecutor[*postmanqv1.Event, *postmanqv1.Event](s.Ctrl)
	s.activityExecutorFactory.EXPECT().Create(senderActivityType).AnyTimes().Return(senderActivityExecutor)
