...
PLUGIN_DIR=$(PROJECT_DIR)/pkg
OUT_DIR=$(PROJECT_DIR)/out
OUT_PLUGIN_DIR=$(OUT_DIR)/plugins
MOCK_DIR=$(PROJECT_DIR)/mock

PLUGINS=$(shell ls -d $(PLUGIN_DIR)/*/ | grep -v plugin)
...
build-plugins:
	...
	$(foreach PLUGIN, $(PLUGINS), $(GOBUILD) -buildmode=plugin -o $(OUT_PLUGIN_DIR)/$(shell basename $(PLUGIN)).so $(PLUGIN_DIR)/$(shell basename $(PLUGIN))/plugin/main.go;)
